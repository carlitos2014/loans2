...
jQuery.fn = jQuery.prototype = {
	...
	pushStack: function (elems) {
		...
		// Añadir el objeto de edad en la pila (como referencia)
		ret.prevObject = esto;
...
var i
	...
	disabledAncestor = addCombinator (
		función (elem) {
			return elem.disabled === true && ("form" en elem || "label" en elem);
		}
...
función Sizzle (selector, contexto, resultados, semilla) {
	...
	// Intente acceder a las operaciones de búsqueda (a diferencia de los filtros) en documentos HTML
	si (! semilla) {
...
isXML = Sizzle.isXML = function (elem) {
	// documentElement se verifica para los casos en que aún no existe
		...
		"no": markFunction (función (selector) {
			...
			volver matcher [expando]?
				markFunction (función (semilla, coincidencias, contexto, xml) {
					...
					// Coincidir con elementos no emparejados por `matcher`
					mientras yo-- ) {
 ...
 * después de que la lista se haya activado de inmediato con el último "memorizado"
...
función adoptValue (valor, resolución, rechazo, noValue) {
	...
	tratar {
		...
		// Otros objetos
			...
			method.call (valor, resolución, rechazo);
		...
		// Otros no-thenables
			...
			resolver.apply (no definido, [valor] .slice (noValue));
		}

	// Para Promesas / A +, convertir excepciones en rechazos
	// Dado que jQuery.when no desenvuelve los problemas, podemos omitir los cheques adicionales que aparecen en
...
jQuery.extend ({
	...
	Aplazado: función (func) {
				...
				pipe: function (/ * fnDone, fnFail, fnProgress * /) {
					...
					return jQuery.Deferred (function (newDefer) {
						jQuery.each (tuplas, función (i, tupla) {
							...
							// Mapear las tuplas (progress, done, fail) a los argumentos (done, fail, progress)
				...
				entonces: function (onFulfilled, onRejected, onProgress) {
					...
					Función de resolución (profundidad, diferido, manejador, especial) {
						función de retorno () {
							var que = esto,
								...
								mightThrow = function () {
									...
									entonces = devuelto &&
										...
										// Solo revisar objetos y funciones para ver si están
										(typeof devuelto === "objeto" ||
											typeof devolvió === "función") &&
										devuelto.
								...
								// Sólo los procesadores normales (resolver) capturan y rechazan las excepciones
								proceso = especial?
									podría
									función () {
										tratar {
							...
							// Volver a resolver las promesas de inmediato para evitar el rechazo falso de
							// errores posteriores
							si (profundidad) {
								proceso();
							} else {
		...
		// Añadir métodos específicos a la lista
		jQuery.each (tuplas, función (i, tupla) {
			var list = tuple [2],
				stateString = tupla [5];
		...
		// Haz de la promesa un aplazamiento
...
jQuery.fn.ready = function (fn) {
	...
	lista lista
		...
		// sucede en el momento de la gestión de errores en lugar de devolución de llamada
		// registro.
		.catch (función (error) {
...
jQuery.extend ({
	...
	listo: función (espera) {
		...
		// Si un evento DOM Ready normal se activa, disminuye y espera si es necesario
...
// Capturar casos donde se llama $ (document) .ready ()
...
función camelCase (cadena) {
	devolver string.replace (rmsPrefix, "ms-") .replace (rdashAlpha, fcamelCase);
}
...
Data.prototype = {
	...
	eliminar: función (propietario, clave) {
		...
		if (clave! == indefinido) {
			...
			// Matriz de llaves o matriz de llaves separadas por espacios
...
//
// 1. Exigir la compatibilidad de la superficie y semántica de la API con la rama 1.9.x
...
// 5. Evite exponer los detalles de la implementación en los objetos de usuario (por ejemplo, expando propiedades)
...
función dataAttr (elem, key, data) {
	...
	// Si no se ha encontrado nada internamente, intenta buscar cualquier
...
jQuery.extend ({
	...
	dequeue: function (elem, type) {
		...
		// Si la cola fx está en cola, siempre elimine el centinela de progreso
		if (fn === "inprogress") {
			fn = queue.shift ();
			startLength--;
		}
...
jQuery.fn.extend ({
	cola: función (tipo, datos) {
		...
		if (argumentos.longitud <definidor) {
			devuelva jQuery.queue (este [0], escriba);
		}
...
var isHiddenWithinTree = function (elem, el) {
		...
		// isHiddenWithinTree podría llamarse desde jQuery # filter function;
...
función adjustCSS (elem, prop, valueParts, tween) {
	...
	if (initialInUnit && initialInUnit [3]! == unidad) {
		...
		while (maxIterations--) {
			...
			// Evalúe y actualice nuestras mejores conjeturas (duplicando las conjeturas que no hacen nada).
			// Finalice si la escala es igual a o cruza 1 (lo que hace que el producto nuevo * no sea positivo).
...
jQuery.fn.extend ({
	show: function () {
		volver showHide (esto, verdadero);
	}
	ocultar: función () {
...
función getAll (contexto, etiqueta) {
	...
	// Use typeof para evitar la invocación del método de argumento cero en los objetos del host (# 15151)
	var ret
...
(función () {
	...
	// Verificar el estado perdido si el nombre está establecido (# 11217)
	...
	// Asegúrese de que textarea (y casilla de verificación) defaultValue esté correctamente clonada
...
función activada (elemento, tipos, selector, datos, fn, uno) {
	var origFn, tipo;
...
/ *
 * Funciones de ayuda para la gestión de eventos, que no forman parte de la interfaz pública.
...
jQuery.event = {
	...
	agregar: función (elemento, tipos, manejador, datos, selector) {
		...
		// Asegúrese de que los selectores no válidos lanzan excepciones en el momento de adjuntar
		// Evaluar contra documentElement en caso de que elem no sea un nodo del elemento (por ejemplo, documento)
		if (selector) {
			jQuery.find.matchesSelector (documentElement, selector);
		}

		// Asegúrese de que el controlador tenga una ID única, que se utiliza para encontrarla o eliminarla más adelante
		if (! handler.guid) {
	...
	dispatch: function (nativeEvent) {
		...
		while ((matched = handlerQueue [i ++]) &&! event.isPropagationStopped ()) {
			...
			while ((handleObj = matched.handlers [j ++]) &&
				...
				// El evento desencadenado debe 1) no tener espacio de nombres, o 2) tener espacio de nombres
	...
	especial: {
		...
		haga clic en: {
			...
			// Para mantener la coherencia entre los navegadores, no active .click () nativo en los enlaces
...
// Crear eventos mouseenter / dejar eventos usando mouseover / out y controles en tiempo de eventos
// para que la delegación de eventos funcione en jQuery.
// Haga lo mismo para el puntero / puntero y el puntero / puntero
//
...
// Reemplazar / restaurar el atributo de tipo de los elementos del script para la manipulación segura de DOM
...
jQuery.extend ({
	htmlPrefilter: function (html) {
		devolver html.replace (rxhtmlTag, "<$ 1> </ $ 2>");
	}
...
jQuery.fn.extend ({
	...
	añadir: function () {
		devuelve domManip (esto, argumentos, función (elem) {
			if (this.nodeType === 1 || this.nodeType === 11 || this.nodeType === 9) {
...
(función () {
	...
	función computeStyleTests () {
		...
		// Esto es un singleton, necesitamos ejecutarlo solo una vez
		si (! div) {
			regreso;
		}
...
función curCSS (elemento, nombre, computado) {
	ancho de var, minWidth, maxWidth, ret,

		// Soporte: Firefox 51+
		// Recuperar el estilo antes computado de alguna manera
	...
	si (calculado) {
		...
		// Esto está en contra de la especificación del borrador de CSSOM:
...
función addGetHookIf (conditionFn, hookFn) {
	...
	// Defina el gancho, verificaremos la primera ejecución si es realmente necesario.
	regreso {
		get: function () {
			if (conditionFn ()) {
...
// Devolver una propiedad css asignada a una propiedad con prefijo de proveedor potencial
...
// Devuelve una propiedad asignada a lo largo de lo que jQuery.cssProps sugiere o
...
función setPositiveNumber (elemento, valor, resta) {
	...
	volver partidos?
		...
		// Protéjase contra "restar" indefinido, por ejemplo, cuando se usa como en cssHooks
...
function boxModelAdjustment (elem, dimension, box, isBorderBox, styles, computedVal) {
	...
	if (caja === (isBorderBox? "border": "content")) {
		devuelve 0;
	}
	...
	para (; i <4; i + = 2) {
		...
		if (! isBorderBox) {
			...
			// Pero aun así, sigue la pista de otra manera.
			} else {
	...
	// Cuenta el contenido de la caja de desplazamiento de la caja de contenido positiva cuando se solicite, proporcionando computedVal
	if (! isBorderBox && computedVal> = 0) {

		// offsetWidth / offsetHeight es una suma redondeada de contenido, relleno, borde de desplazamiento y borde
...
jQuery.extend ({
	...
	// Añadir enganches de propiedad de estilo para anular el valor predeterminado
	...
	css: function (elem, name, extra, styles) {
		...
		// Si se proporcionó un gancho, obtenga el valor computado desde allí
		if (hooks && "get" in hooks) {
			val = hooks.get (elem, true, extra);
		}

		// De lo contrario, si existe una forma de obtener el valor computado, use ese
		if (val === indefinido) {
...
jQuery.each (["altura", "ancho"], función (i, dimensión) {
	jQuery.cssHooks [dimensión] = {
		...
		set: function (elem, value, extra) {
			...
			// Tenga en cuenta las dimensiones no confiables del cuadro de frontera comparando el desplazamiento * con el valor calculado y
			...
			si (restar && (coincidencias = rcssNum.exec (valor)) &&
...
Tween.propHooks = {
	_defecto: {
		get: function (tween) {
			...
			// Usar una propiedad en el elemento directamente cuando no es un elemento DOM,
...
función defaultPrefilter (elem, props, opts) {
	...
	para (prop en props) {
		...
		if (rfxtypes.test (valor)) {
			...
			if (value === (hidden? "hide": "show")) {
				...
				// Ignorar todos los demás datos de mostrar / ocultar no-op
				} else {
					continuar;
				}
			}
	...
	if (isBox && elem.nodeType === 1) {
		...
		// a partir de los valores de overflowX y overflowY con valores idénticos y Edge simplemente duplicados
...
jQuery.Animation = jQuery.extend (Animación, {
	...
	Tweener: función (props, callback) {
		if (isFunction (props)) {
	...
	prefiltro: función (callback, prepend) {
		if (prepend) {
...
jQuery.speed = function (speed, easing, fn) {
	var opt = speed && typeof speed === "object"? jQuery.extend ({}, velocidad): {
		completa: fn || ! fn && facilitando ||
			isFunction (speed) && speed,
		duración: velocidad,
		easing: fn && easing || suavizado &&! isFunction (suavizado) && suavizado
	};
...
jQuery.fn.extend ({
	fadeTo: function (speed, to, easing, callback) {
		...
		// Mostrar los elementos ocultos después de establecer la opacidad a 0
		devuelve this.filter (isHiddenWithinTree) .css ("opacity", 0) .show ()

			// Animate al valor especificado
	...
	animate: function (prop, speed, easing, callback) {
		var empty = jQuery.isEmptyObject (prop),
			...
			DoAnimation = function () {
				...
				// Operar en una copia de prop para que no se pierda la mejora por propiedad
				var anim = Animation (this, jQuery.extend ({}, prop), optall);

				// Vaciar animaciones, o terminar resolviendo inmediatamente.
		...
		volver vacío || optall.queue === false?
			esto. cada (doAnimation):
		...
		devuelve this.each (function () {
			...
			para (index = timers.length; index--;) {
				if (temporizadores [índice] .elem === este &&
					(escriba == null || temporizadores [índice] .queue === tipo)) {
			...
			// Comience el siguiente en la cola si el último paso no fue forzado.
			// Los temporizadores actualmente llamarán a sus devoluciones de llamada completas, que
			// se eliminará de la cola, pero solo si fueran gotoEnd.
			if (dequeue ||! gotoEnd) {
				jQuery.dequeue (this, type);
			}
		});
	...
	acabado: función (tipo) {
		...
		devuelve this.each (function () {
			...
			// Habilitar bandera de acabado en datos privados
			data.finish = true;
			...
			// Busca las animaciones activas, y finalízalas.
			para (index = timers.length; index--;) {
				if (timers [index] .elem === this && timers [index] .queue === type) {
					temporizadores [index] .anim.stop (true);
					timers.splice (índice, 1);
				}
			}

			// Busca las animaciones en la cola antigua y finalízalas.
			para (índice = 0; índice <longitud; índice ++) {
			...
			// Apagar la bandera de acabado
			eliminar data.finish;
		});
	}
});

jQuery.each (["toggle", "show", "hide"], función (i, nombre) {
...
// Generar accesos directos para animaciones personalizadas
jQuery.each ({
...
}, función (nombre, props) {
	jQuery.fn [nombre] = función (velocidad, aceleración, devolución de llamada) {
		devuelve esto. animate (props, speed, easing, callback);
	};
});
...
jQuery.fx.tick = function () {
	...
	para (; i <timers.length; i ++) {
		...
		// Ejecutar el temporizador y eliminarlo de forma segura cuando haya terminado (permitiendo la eliminación externa)
...
// Basado fuera del complemento por Clint Helfers, con permiso.
// https://web.archive.org/web/20100324014747/http://blindsignals.com/index.php/2009/07/jquery-delay/
jQuery.fn.delay = function (time, type) {
	tiempo = jQuery.fx? jQuery.fx.speeds [tiempo] || tiempo tiempo;
	tipo = tipo || "fx";

	devuelve this.queue (tipo, función (siguiente, ganchos) {
...
(función () {
	...
	// Soporte: IE <= 11 solamente
	// Debe acceder a selectedIndex para que las opciones predeterminadas seleccionen
...
jQuery.extend ({
	attr: function (elem, name, value) {
		...
		// Los atributos no existentes devuelven nulo, normalizamos a indefinido
		devuelve ret == null? indefinido: ret;
	}
...
boolHook = {
	set: function (elem, value, name) {
		si (valor === falso) {
			...
			// Eliminar los atributos booleanos cuando se establece en falso
			jQuery.removeAttr (elem, nombre);
		} else {
...
jQuery.each (jQuery.expr.match.bool.source.match (/ \ w + / g), función (i, nombre) {
	...
	attrHandle [nombre] = función (elemento, nombre, isXML) {
		var ret, manejar,
...
jQuery.extend ({
	prop: function (elem, name, value) {
		...
		// No obtener / establecer propiedades en los nodos de texto, comentarios y atributos
		if (nType === 3 || nType === 8 || nType === 2) {
			regreso;
		}
	...
	PropHooks: {
		índice de tabulación: {
			get: function (elem) {
				...
				// Usar la recuperación de atributos adecuada (# 12072)
...
// en la opción
// El getter asegura que se selecciona una opción por defecto
...
if (! support.optSelected) {
	jQuery.propHooks.selected = {
		get: function (elem) {
			...
			/ * eslint expresiones sin usar: "off" * /
...
jQuery.each ([
	...
	// Eliminar y colapsar los espacios en blanco según las especificaciones HTML
...
función getClass (elem) {
	devolver elem.getAttribute && elem.getAttribute ("class") || "";
}

función classesToArray (valor) {
	if (Array.isArray (valor)) {
		valor de retorno;
	}
	if (typeof value === "string") {
		devuelve value.match (rnothtmlwhite) || [];
	}
	regreso [];
}
...
jQuery.fn.extend ({
		...
		if (classes.length) {
			while ((elem = this [i ++])) {
				...
				if (cur) {
					...
					// Solo asigne si es diferente para evitar la representación innecesaria.
	...
	toggleClass: function (value, stateVal) {
		tipo var = valor tipográfico,
			isValidValue = type === "string" || Array.isArray (valor);

		if (typeof stateVal === "boolean" && isValidValue) {
			volver stateVal? this.addClass (value): this.removeClass (value);
		}

		if (isFunction (valor)) {
			devuelve this.each (función (i) {
		...
		devuelve this.each (function () {
			...
			if (isValidValue) {
				...
				while ((className = classNames [i ++])) {
					...
					// Compruebe cada className dado, lista separada por espacios
			...
			// alternar nombre de clase completa
				...
				// Si el elemento tiene un nombre de clase o si se pasa `false`,
				// luego elimine todo el nombre de clase (si hubiera uno, el anterior lo guardó).
				// De lo contrario, traer de vuelta lo que se guardó previamente (en su caso),
...
jQuery.fn.extend ({
	val: función (valor) {
		...
		devuelve this.each (función (i) {
			...
			// Si el conjunto regresa sin definir, vuelve a la configuración normal
		...
		opción: {
			get: function (elem) {
				...
				devuelve val! = null?
					...
					// Separar y colapsar espacios en blanco
			...
			get: function (elem) {
				...
				para (; i <max; i ++) {
					...
					if ((option.selected || i === index) &&
							...
							// No devolver las opciones que están deshabilitadas o en un optgroup deshabilitado
							! option.disabled &&
...
jQuery.each (["radio", "casilla de verificación"], función () {
	...
	si (! support.checkOn) {
		jQuery.valHooks [this] .get = function (elem) {
			devuelve elem.getAttribute ("value") === null? "on": elem.value;
		};
	}
});
...
// Devolver jQuery para inclusión solo de atributos
...
jQuery.extend (jQuery.event, {
	...
	trigger: function (event, data, elem, onlyHandlers) {
		...
		// No hacer eventos en texto y comentar nodos.
		if (elem.nodeType === 3 || elem.nodeType === 8) {
			regreso;
		}

		// enfocar / difuminar los morfos para enfocar / salir; Asegúrate de que no los estamos despidiendo en este momento
		...
		if (type.indexOf (".")> -1) {
			...
			// disparador de espacios de nombre; crear una expresión regular para que coincida con el tipo de evento en handle ()
			espacios de nombres = tipo.split (".");
			type = namespaces.shift ();
			namespaces.sort ();
		}
		ontype = tipo.indexOf (":") <0 && "en" + tipo;

		// El llamador puede pasar un objeto jQuery.Event, un objeto o solo una cadena de tipo de evento
		...
		// Limpiar el evento en caso de que sea reutilizado.
		event.result = undefined;
		if (! event.target) {
			event.target = elem;
		}

		// Clone los datos entrantes y anteponga el evento, creando la lista de argumentos del manejador
		datos = datos == nulo?
			[evento]:
			jQuery.makeArray (datos, [evento]);

		// Permitir eventos especiales para dibujar fuera de las líneas
		special = jQuery.event.special [tipo] || {};
		if (! onlyHandlers && special.trigger && special.trigger.apply (elem, data) === false) {
			regreso;
		}

		// Determine la ruta de propagación de eventos por adelantado, según la especificación de eventos de W3C (# 9951)
		// Bubble hasta el documento, luego a la ventana; Esté atento a una variable global de ownerDocument (# 9724)
		if (! onlyHandlers &&! special.noBubble &&! isWindow (elem)) {
			...
			// Solo agregue una ventana si tenemos que documentar (por ejemplo, no obj simple o DOM separado)
		...
		while ((cur = eventPath [i ++]) &&! event.isPropagationStopped ()) {
			...
			// jQuery handler
			handle = (dataPriv.get (cur, "eventos") || {}) [event.type] &&
		...
		// Si nadie impidió la acción por defecto, hazlo ahora.
		if (! onlyHandlers &&! event.isDefaultPrevented ()) {
			...
			si ((! special._default ||
				...
				if (ontype && isFunction (elem [type]) &&! isWindow (elem)) {
					...
					// Evita que se vuelva a activar el mismo evento, ya que ya lo hicimos anteriormente.
	...
	// Piggyback en un evento donante para simular uno diferente
...
// que es violación de especificaciones - http://www.w3.org/TR/DOM-Level-3-Events/#events-focusevent-event-order
...
if (! support.focusin) {
	jQuery.each ({focus: "focusin", blur: "focusout"}, function (orig, fix) {
		...
		controlador var = función (evento) {
...
jQuery.parseXML = function (data) {
	...
	// Soporte: IE 9 - 11 solamente
	// IE lanza en parseFromString con una entrada no válida.
	tratar {
		xml = (nueva ventana.DOMParser ()) .parseFromString (data, "text / xml");
	} atrapar (e) {
		xml = indefinido;
	}
...
jQuery.param = function (a, traditional) {
	prefijo var
		...
		add = function (key, valueOrFunction) {
			...
			// Si el valor es una función, invócalo y usa su valor de retorno
...
jQuery.fn.extend ({
	...
	serializeArray: function () {
		devuelve this.map (function () {
			...
			var elements = jQuery.prop (esto, "elements");
			elementos de retorno? jQuery.makeArray (elementos): esto;
		})
		.filter (función () {
			tipo var = este tipo;

			// Use .is (": disabled") para que funcione fieldset [disabled]
		...
		.map (función (i, elem) {
			...
			if (Array.isArray (val)) {
				devuelve jQuery.map (val, function (val) {
					devuelve {nombre: elem.nombre, valor: val.replace (rCRLF, "\ r \ n")};
				});
			}
...
var
	 ...
	 * 3) clave es el tipo de datos
	 * 4) se puede utilizar el símbolo de la trampa "*"
	 * 5) la ejecución comenzará con el tipo de datos de transporte y, luego, continuará hasta "*" si es necesario
	 * /
	prefiltros = {},

	/ * Transportes de encuadernaciones
	 * 1) clave es el tipo de datos
	 * 2) Se puede usar el símbolo de la trampa "*"
	 * 3) la selección comenzará con el tipo de datos de transporte y luego ir a "*" si es necesario
	 * /
	transportes = {},

	// Evita la secuencia de caracteres de comentario-prólogo (# 10098); Debe apaciguar la pelusa y evadir la compresión.
	allTypes = "* /". concat ("*"),
...
función addToPrefiltersOrTransports (estructura) {
	...
	función de retorno (dataTypeExpression, func) {
		...
		if (isFunction (func)) {
			...
			// Para cada dataType en el dataTypeExpression
			while ((dataType = dataTypes [i ++])) {

				// Prefiere si se solicita
				if (dataType [0] === "+") {
					dataType = dataType.slice (1) || "*";
					(estructura [dataType] = estructura [dataType] || []) .unshift (func);

				// de lo contrario adjuntar
				} else {
					(estructura [dataType] = estructura [dataType] || []) .push (func);
				}
			}
		}
	};
...
function inspectPrefiltersOrTransports (estructura, opciones, opciones originales, jqXHR) {
	...
	inspección de devolución (options.dataTypes [0]) || ! inspeccionado ["*"] && inspeccionar ("*");
}

// Una extensión especial para las opciones de ajax.
// que toma opciones "planas" (no se extienden en profundidad)
// Arreglos # 9887
función ajaxExtend (target, src) {
	clave var, profunda,
...
función ajaxHandleResponses (s, jqXHR, respuestas) {
	...
	si (ct) {
		para (escriba en los contenidos) {
	...
	// Compruebe si tenemos una respuesta para el dataType esperado
	if (dataTypes [0] en las respuestas) {
		finalDataType = dataTypes [0];
	} else {

		// Probar tipos de datos convertibles
	...
	// Añadimos el dataType a la lista si es necesario
...
función ajaxConvert (s, respuesta, jqXHR, isSuccess) {
	...
	// Crear un mapa de convertidores con claves en minúscula
	if (dataTypes [1]) {
	...
	while (actual) {
		...
		if (actual) {
			...
			// Solo hay trabajo que hacer si el dataType actual no es automático
			if (actual === "*") {

				current = prev;

			// Convertir la respuesta si el tipo de datos anterior es no automático y difiere del actual
			} else if (prev! == "*" && prev! == current) {

				// Busca un convertidor directo
				conv = converters [prev + "" + current] || convertidores ["*" + actual];

				// Si no encuentra ninguno, busque un par
				si (! conv) {
					para (conv2 en convertidores) {

						// Si conv2 saca corriente
						tmp = conv2.split ("");
						if (tmp [1] === actual) {

							// Si prev puede ser convertido a entrada aceptada
				...
				if (conv! == true) {
					...
					// A menos que se permita que los errores formen burbujas, atrapen y devuelvan
					if (conv && s.throws) {
						...
						tratar {
							...
							regreso {
								estado: "parsererror",
								error: conv? e: "No hay conversión de" + prev + "a" + actual
							};
						}
					}
				}
...
jQuery.extend ({
	...
	// Si se omite el destino, escribe en ajaxSettings.
	...
	ajax: función (url, opciones) {
		...
		// Si url es un objeto, simule una firma pre-1.5
		...
		transporte var
			...
			// URL sin anti-caché param
			caché

			// Encabezados de respuesta
			respuestaHeadersString,
			Cabezas de respuesta,

			// tiempo de espera manejar
			tiempo de espera
			...
			// parte de la url sin caché
			sin caché

			// Crear el objeto de opciones final.
			...
			// Aplazado
			aplazado = jQuery.Deferred (),
			completeDeferred = jQuery.Callbacks ("once memory"),

			// Callbacks dependientes del estado
			...
			jqXHR = {
				...
				// Construye encabezados hashtable si es necesario
				getResponseHeader: function (key) {
					...
					devuelve partido == nulo? nulo: coincidencia;
				}
				...
				código de estado: función (mapa) {
					...
					si (mapa) {
						si (completado) {
							...
							// Perezoso: agregue las nuevas devoluciones de llamada de una manera que conserve las antiguas
		...
		// Agregar protocolo si no se proporciona (los prefiltros lo pueden esperar)
		...
		// Opción de método alias para escribir según el boleto # 12004
		s.type = options.method || opciones.tipo || en el método || tipo de

		// Extraer dataTypes list
		s.dataTypes = (s.dataType || "*") .toLowerCase (). match (rnothtmlwhite) || [""];

		// Una solicitud de dominio cruzado está en orden cuando el origen no coincide con el origen actual.
		if (s.crossDomain == null) {
			urlAnchor = document.createElement ("a");

			// Soporte: IE <= 8 - 11, Edge 12 - 15
			// IE lanza una excepción al acceder a la propiedad href si la url tiene un formato incorrecto,
			// p.ej. http://ejemplo.com:80x/
			tratar {
				urlAnchor.href = s.url;

				// Soporte: IE <= 8 - 11 solamente
				// La propiedad del host del ancla no se establece correctamente cuando s.url es relativo
		...
		// Si la solicitud fue abortada dentro de un prefiltro, pare ahí
		si (completado) {
			devuelve jqXHR;
		}

		// Podemos lanzar eventos globales a partir de ahora si se nos pide
		...
		// Determine si la solicitud tiene contenido
		s.hasContent =! rnoContent.test (s.type);

		// Guarda la URL en caso de que estemos jugando con el If-Modified-Since
		// y / o If-None-Match cabecera más adelante
		...
		if (! s.hasContent) {
			...
			// Si los datos están disponibles y deberían procesarse, agregue los datos a la url
			if (s.data && (s.processData || typeof s.data === "string")) {
				cacheURL + = (rquery.test (cacheURL)? "&": "?") + s.data;

				// # 9682: eliminar datos para que no se utilicen en un posible reintento
				eliminar s.data;
			}

			// Agregar o actualizar el parámetro anti-cache si es necesario
			if (s.cache === false) {
				cacheURL = cacheURL.replace (rantiCache, "$ 1");
				uncached = (rquery.test (cacheURL)? "&": "?") + "_ =" + (nonce ++) + uncached;
			}

			// Poner hash y anti-cache en la URL que será solicitada (gh-1732)
			s.url = cacheURL + uncached;

		// Cambie '% 20' a '+' si esto está codificado en el contenido del cuerpo del formulario (gh-2658)
		} else if (s.data && s.processData &&
			(s.contentType || "") .indexOf ("application / x-www-form-urlencoded") === 0) {
			s.data = s.data.replace (r20, "+");
		}

		// Establecer el encabezado If-Modified-Since y / o If-None-Match, si está en el modo ifModified.
		si (si está modificado) {
		...
		// Establecer el encabezado correcto, si se están enviando datos
		...
		si (! transporte) {
			...
			// Si la solicitud fue abortada dentro de ajaxSend, detente ahí
			si (completado) {
				devuelve jqXHR;
			}
			...
			tratar {
				...
				// Volver a eliminar las excepciones posteriores a la finalización
				si (completado) {
					lanzar e;
				}
		...
		función realizada (status, nativeStatusText, respuestas, encabezados) {
			...
			// Transporte de desreferencia para recolección temprana de basura.
			...
			// Convertir sin importar qué (de esta manera, los campos de respuestaXXX siempre se configuran)
			response = ajaxConvert (s, response, jqXHR, isSuccess);

			// Si tiene éxito, manejar el encadenamiento de tipos
			if (isSuccess) {
				...
				// Extraer error de statusText y normalizar para no abortar
				error = statusText;
				si (estado ||! estadoTexto) {
					statusText = "error";
					si (estado <0) {
						estado = 0;
					}
			...
			if (fireGlobals) {
				...
				// Manejar el contador global de AJAX
				if (! (--jQuery.active)) {
	...
	getScript: function (url, callback) {
		devolver jQuery.get (url, undefined, callback, "script");
	}
});
...
jQuery.each (["get", "post"], function (i, method) {
	jQuery [método] = función (url, datos, devolución de llamada, tipo) {
		...
		// Desplazar argumentos si se omitió el argumento de datos
		if (isFunction (data)) {
...
var xhrSuccessStatus = {
		...
		// El protocolo de archivo siempre produce el código de estado 0, suponiendo 200
		0: 200,
...
jQuery.ajaxTransport (función (opciones) {
	...
	// Solo se permite el dominio cruzado si se admite a través de XMLHttpRequest
	if (support.cors || xhrSupported &&! options.crossDomain) {
		regreso {
			enviar: función (encabezados, completa) {
				...
				// Anular el tipo mime si es necesario
				if (options.mimeType && xhr.overrideMimeType) {
					xhr.overrideMimeType (options.mimeType);
				}

				// X-Requested-con encabezado
				// Para solicitudes de varios dominios, considerando que las condiciones para una verificación previa son
				// similar a un rompecabezas, simplemente nunca lo configuramos para estar seguro.
				// (siempre se puede establecer por solicitud o incluso utilizando ajaxSetup)
					...
					función de retorno () {
						si (devolución de llamada) {
							...
							if (tipo === "abortar") {
								...
								// errores en cualquier acceso de propiedad que no sea readyState
								...
								completar(
									...
									// Para XHR2 no textual, deje que la persona que llama lo maneje (gh-2498)
				...
				// para manejar los abortos no capturados
				if (xhr.onabort! == indefinido) {
					xhr.onabort = errorCallback;
				} else {
					xhr.onreadystatechange = function () {

						// Compruebe readyState antes de que se agote el tiempo de espera
						if (xhr.readyState === 4) {

							// Permitir que se llame primero a onerror,
							// pero eso no va a manejar un aborto nativo
							// También, guarda errorCallback en una variable
							// como xhr.onerror no se puede acceder
							window.setTimeout (function () {
								si (devolución de llamada) {
									errorCallback ();
								}
				...
				// Crear la devolución de llamada abortar
				devolución de llamada = devolución de llamada ("abortar");

				tratar {

					// Enviar la solicitud (esto puede generar una excepción)
					xhr.send (options.hasContent && options.data || null);
				} atrapar (e) {

					// # 14683: volver a emitir solo si esto aún no se ha notificado como un error
					si (devolución de llamada) {
						lanzar e;
					}
				}
...
// Impedir la ejecución automática de scripts cuando no se proporcionó un dataType explícito (Ver gh-2432)
...
// Etiqueta de script de enlace hackear transporte
jQuery.ajaxTransport ("script", función (es) {

	// Este transporte solo trata con peticiones de dominios cruzados
	if (s.crossDomain) {
		script var, devolución de llamada;
		regreso {
			enviar: función (_, completa) {
				script = jQuery ("<script>") .prop ({
					conjunto de caracteres: s.scriptCharset,
					src: s.url
				} ).en(
					"Error de carga",
...
// Configuracion jsonp predeterminada
jQuery.ajaxSetup ({
	jsonp: "devolución de llamada",
	jsonpCallback: function () {
		var callback = oldCallbacks.pop () || (jQuery.expando + "_" + (nonce ++));
		este [callback] = verdadero;
...
jQuery.ajaxPrefilter ("json jsonp", función (s, configuración original, jqXHR) {
	...
	if (jsonProp || s.dataTypes [0] === "jsonp") {
		...
		// Insertar callback en url o datos de formulario
		if (jsonProp) {
		...
		// Usar el convertidor de datos para recuperar json después de la ejecución del script
		...
		// Función de limpieza (incendios después de los convertidores)
		jqXHR.always (function () {
			...
			// Llame si era una función y tenemos una respuesta.
...
// Soporte: solo para Safari 8
// En los documentos de Safari 8 creados a través de document.implementation.createHTMLDocument
// colapsar formas de hermanos: el segundo se convierte en un hijo del primero.
// Debido a esto, esta medida de seguridad debe estar deshabilitada en Safari 8.
...
jQuery.parseHTML = function (data, context, keepScripts) {
	...
	si (! contexto) {
		...
		// Detiene la ejecución inmediata de scripts o controladores de eventos en línea
		// utilizando document.implementation
		if (support.createHTMLDocument) {
			context = document.implementation.createHTMLDocument ("");

			// Establecer la base href para el documento creado
...
jQuery.fn.load = function (url, params, callback) {
	...
	if (self.length> 0) {
		jQuery.ajax ({
			...
			// Hacer explícito el valor de este campo desde
			...
			// Guardar respuesta para su uso en la devolución de llamada completa
			respuesta = argumentos;

			self.html (selector?

				// Si se especificó un selector, ubique los elementos correctos en un dummy div
...
// Adjuntar un montón de funciones para manejar eventos AJAX comunes
jQuery.each ([
	"ajaxStart",
	"ajaxStop",
	"ajaxComplete",
...
jQuery.offset = {
	setOffset: function (elem, options, i) {
		var curPosition, curLeft, curCSSTop, curTop, curOffset, curCSSLeft, calculaPosition,
			position = jQuery.css (elem, "position"),
			curElem = jQuery (elem),
			props = {};

		// Establezca la posición primero, en caso de que la parte superior / izquierda se establezcan incluso en elem estático
		if (posición === "estática") {
		...
		CalculatePosition = (posición === "absoluta" || posición === "fijo") &&
			(curCSSTop + curCSSLeft) .indexOf ("auto")> -1;

		// Necesito poder calcular la posición si
		// arriba o izquierda es auto y la posición es absoluta o fija
		if (calcularPosicion) {
...
jQuery.fn.extend ({
	...
	desplazamiento: función (opciones) {
		...
		// nodo desconectado en IE lanza un error
		if (! elem.getClientRects (). length) {
			return {top: 0, left: 0};
		}

		// Obtenga la posición relativa al documento agregando el desplazamiento de la vista al gBCR relativo a la vista
	...
	posición: función () {
		...
		// posición: los elementos fijos están desplazados desde la ventana gráfica, que a su vez siempre tiene cero desplazamiento
		if (jQuery.css (elem, "posición") === "arreglado") {

			// Asumir posición: fijo implica la disponibilidad de getBoundingClientRect
			offset = elem.getBoundingClientRect ();

		} else {
			offset = this.offset ();

			// Cuenta para el padre * real * offset, que puede ser el documento o su elemento raíz
		...
		regreso {
			...
			izquierda: offset.left - parentOffset.left - jQuery.css (elem, "marginLeft", verdadero)
		};
	}
	...
	// documentElement de la ventana padre
	// 2) Para el elemento oculto o separado
	// 3) Para el cuerpo o el elemento html, es decir, en el caso del nodo html, se devolverá solo.
	//
...
jQuery.each ({scrollLeft: "pageXOffset", scrollTop: "pageYOffset"}, function (method, prop) {
	...
	jQuery.fn [método] = función (val) {
		acceso de retorno (esta, función (elemento, método, val) {

			// Documentos y ventanas coalescentes.
			var ganar
			if (isWindow (elem)) {
				win = elem;
			...
			if (val === indefinido) {
				volver ganar? win [prop]: elem [method];
			}
		...
		}, metodo, val, argumentos.longitud);
	};
});

// Soporte: Safari <= 7 - 9.1, Chrome <= 37 - 49
// Agregar los cssHooks superiores / izquierdos usando jQuery.fn.position
// Error del Webkit: https://bugs.webkit.org/show_bug.cgi?id=29084
...
// en lugar de hacer que el módulo css dependa del módulo de desplazamiento, solo compruébelo aquí
	...
	jQuery.cssHooks [prop] = addGetHookIf (support.pixelPosition,
		función (elem, computada) {
			si (calculado) {
				...
				// Si curCSS devuelve porcentaje, retroceso a desplazamiento
				devuelve rnumnonpx.test (calculado)?
...
// Crear métodos de altura interna, anchura interna, altura, anchura, altura externa y anchura exterior
jQuery.each ({Altura: "altura", Ancho: "ancho"}, función (nombre, tipo) {
	jQuery.each ({padding: "inner" + name, content: type, "": "outside" + name},
		función (defaultExtra, funcName) {

		// El margen es solo para externalHeight, outerWidth
		jQuery.fn [funcName] = función (margen, valor) {
			var chainable = argumentos.longitud && (defaultExtra || tipo de margen! == "booleano"),
				extra = defaultExtra || (margen === verdadero || valor === verdadero? "margen": "borde");

			acceso de retorno (esta, función (elemento, tipo, valor) {
				var doc;

				if (isWindow (elem)) {

					// $ (ventana) .outerWidth / Height return w / h incluyendo barras de desplazamiento (gh-1729)
					devuelve funcName.indexOf ("outside") === 0?
				...
				if (elem.nodeType === 9) {
					...
					// Desplazar [Ancho / Alto] o desplazar [Ancho / Alto] o cliente [Ancho / Alto],
					// lo que sea mas grande
					volver Math.max (
				...
				valor de retorno === indefinido?
					...
					// Obtener ancho o alto en el elemento, solicitando pero sin forzar parseFloat
					jQuery.css (elem, tipo, extra):

					// Establecer ancho o alto en el elemento
					jQuery.style (elemento, tipo, valor, extra);
			}, tipo, chainable? margen: indefinido, chainable);
		};
	});
});
...
// Enlazar una función a un contexto, opcionalmente aplicando parcialmente
// argumentos.
// jQuery.proxy está en desuso para promover estándares (específicamente la función # bind)
// Sin embargo, no está programado para ser retirado pronto
jQuery.proxy = function (fn, context) {
	...
	// Verificación rápida para determinar si el objetivo es llamable, en la especificación
	// esto genera un TypeError, pero solo devolveremos undefined.
	if (! isFunction (fn)) {
		regreso indefinido;
	}
	...
	proxy = función () {
		devuelve fn.apply (context || this, args.concat (slice.call (argumentos)));
	};

	// Establezca el guid de controlador único en el mismo controlador original, para que pueda eliminarse
...
jQuery.isNumeric = function (obj) {
	...
	// A partir de jQuery 3.0, isNumeric se limita a
	// cadenas y números (primitivos u objetos)
	// que puede ser forzado a números finitos (gh-2662)
	tipo var = jQuery.type (obj);
	return (escriba === "número" || escriba === "cadena") &&
...
// los archivos que pueden usar definen, pero no a través de un script de concatenación adecuado que
// entiende los módulos anónimos de AMD. Un AMD llamado es el más seguro y robusto.
// forma de registrarse. Se utiliza jquery en minúscula porque los nombres de los módulos AMD son
...
// Tenga en cuenta que para una portabilidad máxima, las bibliotecas que no son jQuery deberían
// se declaran a sí mismos como módulos anónimos, y evite establecer un global si un
